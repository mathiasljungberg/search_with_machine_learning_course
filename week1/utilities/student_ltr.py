...
def extract_logged_features(hits, query_id):
    ...
    features_default = {k:[] for k in ["doc_id", "query_id", "sku"]}
    features_extra =  {d['name']:[] for d in hits[0]['fields']['_ltrlog'][0]['log_entry']}
    feature_results = {**features_default, **features_extra}

    for (idx, hit) in enumerate(hits):
        feature_results["doc_id"].append(int(hit.get('_id', 0)))  # capture the doc id so we can join later
        feature_results["query_id"].append(query_id)  # super redundant, but it will make it easier to join later
        feature_results["sku"].append(int(hit.get('_id', 0)))

        for k in features_extra:
            for l in hit['fields']['_ltrlog'][0]['log_entry']:
                if l['name'] == k:
                    feature_results[k].append(l.get('value',0))

    frame = pd.DataFrame(feature_results)
    types_default = {'doc_id': 'int64', 'query_id': 'int64', 'sku': 'int64'}
   
    return frame.astype(types_default)
